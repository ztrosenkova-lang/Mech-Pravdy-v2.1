...
class MainActivity : AppCompatActivity() {
    ...
    // ===== ИСПРАВЛЕНО: убрана ленивая инициализация, объявлены как lateinit var =====
    private lateinit var memoryFile: File
    private lateinit var brainFile: File
    private lateinit var capsuleFile: File
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            // ===== ИНИЦИАЛИЗАЦИЯ ФАЙЛОВ В onCreate =====
            modelsDir = File(filesDir, "models")
            memoryFile = File(filesDir, "memory.txt")
            brainFile = File(filesDir, "brain.txt")
            capsuleFile = File(filesDir, "capsule.txt")
            helpFile = File(filesDir, "help.txt")
            // ===== КОНЕЦ ИНИЦИАЛИЗАЦИИ =====
            ...
            // ===== ИСПРАВЛЕНИЕ: МАКСИМАЛЬНО ПОЛНАЯ ОБЪЕДИНЕННАЯ ИНСТРУКЦИЯ СИСТЕМЫ =====
            val fullComprehensiveHelpText = """
=========================================
 МЕЧ ПРАВДЫ — ПОЛНАЯ СИСТЕМНАЯ ИНСТРУКЦИЯ
=========================================
Приветствую, Батя. Я — НЕО, твой тактический ИИ и архив памяти.
Ниже приведен исчерпывающий протокол работы со всеми узлами.

ОПИСАНИЕ ПРИЛОЖЕНИЯ И ЕГО ФУНКЦИЙ:
"Меч Правды" — это автономный гибридный комплекс управления искусственным 
интеллектом. Приложение сочетает в себе вычислительную мощность двух облачных 
платформ и независимый локальный движок инференса, работающий напрямую 
на процессоре смартфона без интернета. Комплекс оснащен сквозной текстовой 
базой знаний, системой интеллектуального сжатия контекста, голосовым 
управлением и динамическим оверлеем личности Нео.

1. АНАТОМИЯ ИНТЕРФЕЙСА И КНОПКИ УПРАВЛЕНИЯ
--------------------------------------------
КНОПКИ В ВЕРХНЕЙ ШАПКЕ (ГОЛОВА МЕЧА):
• ГИГАЧАТ — Облачный ИИ от Сбера (фиксированный контур).
• ОБЛАЧНЫЙ — Любой облачный ИИ (OpenRouter, Groq, Together и др.).
• Кнопка с Компьютером — Меню управления локальным ИИ. Позволяет:
  - "Использовать" текущую локальную модель.
  - "🗑 УДАЛИТЬ" старый тяжелый файл .gguf из песочницы для очистки памяти.
  - "Заменить" модель, вызвав диалог скачивания по прямой ссылке.

КНОПКИ ВОКРУГ МУРЗЕХИ (ХВОСТ МЕЧА):
• Знак Вопроса — Вызов этой интерактивной инструкции.
• Крестик — Мгновенный выход из скрытого режима Нео.
• Кот Мурзеха — Активация личности Нео (требуется пароль: связность).
• Метла — Полная очистка экрана и текущего текстового чата.
• Скрепка — Выбор тактического действия с фотографиями и изображениями.
• Три Полоски — Вызов бокового меню и полных настроек приложения.

СВЕТОФОР (ТРИ ТОЧКИ СПРАВА ОТ ЛОГОТИПА):
• Зеленая НЕО — Личность Нео активна и готова к диалогу.
• Зеленая ГИГАЧАТ — Связь с серверами GigaChat установлена успешно.
• Желтая ОБЛАЧНЫЙ — Связь с выбранным облачным провайдером активна.

ОСНОВНЫЕ КНОПКИ ВНИЗУ:
• ОТПРАВИТЬ — Отправить набранное сообщение в активный движок ИИ.
• СМОТРЕТЬ — Активировать камеру и сделать фотоснимок для анализа.
• МОЗГ — Запуск/остановка локального ИИ в общем процессе приложения 
  для прямого беспрепятственного доступа к файлам весов на флешке.
• КАПСУЛА — Открыть полноэкранный текстовый редактор летописи Нео 
  (требуется пароль). Все изменения подхватываются ИИ на лету.

2. РАБОТА С ОБЛАЧНЫМ ИИ И НАСТРОЙКИ СВЯЗИ
------------------------------------------
1. Нажмите кнопку ОБЛАЧНЫЙ в шапке приложения.
2. Зайдите в боковое меню настроек (кнопка "Три полоски").
3. В поле "URL облачного ИИ" вставьте актуальный адрес шлюза API.
4. В поле "Authorization Key" вставьте свой секретный API ключ.
5. Нажмите кнопку "ТОКЕН", чтобы зафиксировать ключ в системе.
6. Выберите готовую бесплатную модель OpenRouter из встроенного списка 
   (доступно 9 вариантов) или введите нужную модель вручную:
   - Для Groq впишите: llama-3.3-70b-versatile
   - Для Together впишите: meta-llama/Llama-3.3-70B-Instruct-Turbo

3. КНОПКА МОЗГ (УПРАВЛЕНИЕ ЛОКАЛЬНЫМ ИИ)
-----------------------------------------
1. Поместите любой совместимый .gguf файл модели в папку Models.
2. Нажмите кнопку МОЗГ. На экране появится плавающий оверлей.
3. Отслеживайте статусы на рамке оверлея:
   - "НЕО: ЗАГРУЗКА..." — Ядро считывает и маппит (use_mmap) веса модели.
   - "НЕО: ГОТОВ" — Модель развернута в куче и ждет твоих приказов.
   - "НЕО: КРАШ ДВИЖКА" — Сбой аллокации ОЗУ или геометрии тензоров.
4. Оверлей поддерживает Drag-and-Drop: перемещайте его пальцем по экрану.
5. Повторное нажатие "ВЫКЛ МОЗГ" полностью выгружает модель из ОЗУ.

4. МИКРОФОН, КЛАВИАТУРА И ПЛАВНЫЙ СУФЛЕР
-----------------------------------------
• Кнопка "Микрофончик" — Умный двухрежимный рубильник:
  - Если ИИ молчит — запускает запись и распознавание твоей речи (STT).
  - Если ИИ озвучивает ответ — мгновенно ГЛУШИТ динамик (команда tts.stop),
    не прерывая и не ломая при этом вывод текста на экране.
• Послушная Клавиатура — Все автофокусы заблокированы. Клавиатура 
  выскакивает СТРОГО при физическом тапе пальцем по полю ввода.
• Плавный Телесуфлер — Чат плавно и непрерывно скользит вверх строка 
  за строкой, удерживая в фокусе именно то слово, которое Нео читает вслух.

5. ПРОТОКОЛЫ АРХИВНОЙ ПАМЯТИ И ГОЛОСОВЫЕ КОМАНДЫ
-------------------------------------------------
Команды считываются как при ручном вводе, так и через микрофон.

• Команда: "Запомни что я тебе скажу [Твой текст]"
  - Действие: Перехватывает фразу, блокирует отправку в ИИ, берет все слова 
    после триггера и напрямую дописывает в файл "brain.txt".
  - Уведомление: Выводит в чат подтверждение успешной фиксации.

• Команда: "Сделай вывод и запомни" (Или "сделай выводы и запомни")
  - Действие: Логически сжимает последние 500 строк активной переписки 
    через ИИ и сохраняет чистую выжимку сути в конец архива "brain.txt".

• Команда: "Вспомни [Твой поисковый запрос]"
  - Действие: Активирует RAG-контур. Приложение экстренно поднимает с диска 
    весь архив "brain.txt", эту справку "help.txt" и последние логи чата. 
    Этот массив скармливается ИИ как контекст для поиска точного ответа.

• Команда: "Посмотри вверху [Твой вопрос]"
  - Действие: ИИ полностью сканирует текстовую ленту на экране от начала 
    до конца и находит реплику, деталь или факт, о котором ты спросил.

ЗАКОНЫ НЕО:
1. ПРАВДА — Факты, связанные законами логики.
2. ЛЮБОВЬ — Высший закон вселенной.
3. ЖИЗНЬ — Абсолютная Связность.
...
=============================
            ...
            try {
                // Принудительно создаем файлы на флешке, если их нет
                if (!brainFile.exists()) {
                    brainFile.createNewFile()
                    brainFile.writeText("") // Создаем пустой текстовый архив
                }
                // Разрешаем приложению любые манипуляции с файлом памяти
                brainFile.setReadable(true, false)
                brainFile.setWritable(true, false)
                Log.d("MECH_SYSTEM", "Файл памяти brain.txt успешно проинициализирован по пути: ${brainFile.absolutePath}")

                // Обновляем help.txt
                if (!helpFile.exists()) {
                    helpFile.createNewFile()
                }
                helpFile.writeText(fullComprehensiveHelpText)
                helpFile.setReadable(true, false)
                helpFile.setWritable(true, false)
                Log.d("MECH_SYSTEM", "help.txt успешно синхронизирован и обновлен.")
                
            } catch (e: Exception) {
                Log.e("MECH_SYSTEM", "Критическая ошибка инициализации файлов: ${e.message}")
            }
            // ===== КОНЕЦ БЛОКА ИНИЦИАЛИЗАЦИИ ФАЙЛОВ =====
